...
reqeust_interrupt (gpointer thread_info, HANDLE native_thread_handle, gint32 pending_apc_slot, PAPCFUNC apc_callback, DWORD tid)
{
	...
	* and complete the mono iterrupt/abort request. In such cases, just keep on
	...
	* This is done to prevent races between interrupt/abort occuring just before thread enters an
...
leave_alertable_wait (MonoThreadInfo *info)
{
	...
	InterlockedExchange (&info->thread_wait_info, 0);
}
