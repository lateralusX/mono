...
#if defined(TARGET_AMD64) && defined(TARGET_WIN32) && defined(HOST_WIN32)
...
emit_library_info (MonoAotCompile *acfg)
{
	...
	if (link_shared_library (acfg)) {
		...
		// Simple implementation just returning TRUE.
		fprintf (acfg->fp, ".cfi_startproc\n");
		fprintf (acfg->fp, "movl $1, %%eax\n");
		fprintf (acfg->fp, "ret\n");
		fprintf (acfg->fp, ".cfi_endproc\n");
