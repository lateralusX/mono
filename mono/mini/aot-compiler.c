...
get_cached_unwind_info_section_item_win32 (MonoAotCompile *acfg, const char *function_start, const char *function_end, GSList *unwind_ops)
{
	...
	if (acfg->unwind_info_section_cache) {
		...
		if (item) {
			...
			// Prepend to begining of list to speed up inserts.
...
// Emit unwind info sections for each fuction. Unwind info on Windows x64 is emitted into two different sections.
// .pdata includes the serialized DWORD aligend RVA's of function start, end and address of serialized
// UNWIND_INFO struct emitted into .xdata, see https://msdn.microsoft.com/en-us/library/ft9x1kdx.aspx.
// .xdata section includes DWORD aligened serialized version of UNWIND_INFO struct, https://msdn.microsoft.com/en-us/library/ddssxxy8.aspx.
static void
emit_unwind_info_sections_win32 (MonoAotCompile *acfg, const char *function_start, const char *function_end, GSList *unwind_ops)
{
