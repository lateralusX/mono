...
#define DW_CFA_mono_sp_alloc_info (DW_CFA_lo_user + 1)
#define DW_CFA_mono_fp_alloc_info (DW_CFA_lo_user + 2)
#endif
...
#if defined(TARGET_WIN32) && defined(TARGET_AMD64)
#define mono_emit_unwind_op_sp_alloc(cfg,ip,size) mono_emit_unwind_op (cfg, (ip) - (cfg)->native_code, DW_CFA_mono_sp_alloc_info, 0, (size))
#define mono_emit_unwind_op_fp_alloc(cfg,ip,reg,size) mono_emit_unwind_op (cfg, (ip) - (cfg)->native_code, DW_CFA_mono_fp_alloc_info, (reg), (size))
#else
...
#if defined(TARGET_WIN32) && defined(TARGET_AMD64)
#define mono_add_unwind_op_sp_alloc(op_list,code,buf,size) do { (op_list) = g_slist_append ((op_list), mono_create_unwind_op ((code) - (buf), DW_CFA_mono_sp_alloc_info, 0, (offset))); } while (0)
#define mono_add_unwind_op_fp_alloc(op_list,code,buf,reg,size) do { (op_list) = g_slist_append ((op_list), mono_create_unwind_op ((code) - (buf), DW_CFA_mono_fp_alloc_info, (reg), (offset))); } while (0)
#else
